...
public class CloseableURLClassLoader extends URLClassLoader {
  ...
  private static class CloseableJarURLConnection extends JarURLConnection {
    ...
    public CloseableJarURLConnection(URL url, JarFile jarFile)
  ...
  private static class CloseableJarFileLoader {
    ...
    public URL getURL(String name) {
      if (jarFile.getEntry(name) != null)
        try {
          return new URL(JAR, null, -1, jarFileURLPrefixString + name,
            jarURLStreamHandler);
  ...
  Class defineClass(String name, URL resourceURL, Manifest manifest)
    throws IOException {
    ...
    if (lastDot != -1) {
      ...
      if (pkg != null) {
        checkForSealedPackage(pkg, packageName, manifest, connection
          .getJarFileURL());
      } else {
    ...
    try {
      ...
      CodeSource cs = new CodeSource(connection.getJarFileURL(), entry
        .getCertificates());
  ...
  private void checkForSealedPackage(Package pkg, String packageName,
    Manifest manifest, URL jarFileURL) {
    if (pkg.isSealed() && !pkg.isSealed(jarFileURL))
      throw new SecurityException(
        "The package '" + packageName + "' was previously loaded and is already sealed."); //$NON-NLS-2$
    ...
    if (Boolean.valueOf(sealed).booleanValue())
      throw new SecurityException(
        "The package '" + packageName + "' was previously loaded unsealed. Cannot seal package."); //$NON-NLS-2$
  }
      ...
      public Object run() {
        synchronized (loaders) {
          if (closed)
            return null;
          for (Iterator iterator = loaders.iterator(); iterator.hasNext();) {
            CloseableJarFileLoader loader = (CloseableJarFileLoader) iterator
              .next();
      ...
      if (isFileJarURL(url)) {
        if (closed)
          throw new IllegalStateException(
            "Cannot add url. CloseableURLClassLoader is closed.");
        loaderURLs.add(url);
        safeAddLoader(url);
        return;
      }
